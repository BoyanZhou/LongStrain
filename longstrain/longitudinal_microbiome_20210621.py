...
class Subject:
    ...
    def strain_initial(self, chromosome):
        ...
        target_bins = get_nonzero_quantile_bins(reads_bin_all_sample)
...
def fqs_species_process(subject_name1, fqs1, species_ref1, bowtie_ref1, output_path="./"):
    """
    Process fqs with known species name, one species a time
    :param subject_name1:
    :param fqs1: fqs is like [[a1_fq,a2_fq],[b1_fq,b2_fq],[c1_fq,c2_fq]]
    :param species_ref1: fas reference of one species
    :param bowtie_ref1: corresponding bowtie ref of that fas
    :param output_path:
    :return:
    """
    # ### change to output path ###
    os.chdir(output_path)
    print(f"subject name is: {subject_name1}\nfastq list is: {fqs1}\nmapping reference is: {species_ref1}\n"
          f"output path is: {output_path}")
    subject1 = Subject(subject_name1, species_ref1)
    # add fastqs
    for i in fqs1:
        subject1.add_fqs(i)
    bam_list = subject1.fqs_to_bams(bowtie_ref1)
    # bam_list = ["Bifidobacterium_breve_simu2b_t0_q20_sorted_rmdup.bam", "Bifidobacterium_breve_simu2b_t1_q20_sorted_rmdup.bam", "Bifidobacterium_breve_simu2b_t2_q20_sorted_rmdup.bam"]
    # bam_list = ["Bifidobacterium_breve_repeated10_t0_q20_sorted_rmdup.bam", "Bifidobacterium_breve_repeated10_t1_q20_sorted_rmdup.bam", "Bifidobacterium_breve_repeated10_t2_q20_sorted_rmdup.bam"]
    subject1.add_bams(bam_list)
    subject1.coverage_summary()
    subject1.strain_identification(subject_name1)


if __name__ == "__main__":
    # identify with sample list
    # subject_id1 = sys.argv[1]
    # sample_list1 = sys.argv[2].split(",")
    # reference1 = sys.argv[3]
    # out_name1 = sys.argv[4]
    # out_name2 = sys.argv[5]
    # out_name3 = sys.argv[6]
    # subject_temp = Subject(subject_id1, reference1)
    # subject_temp.add_samples(sample_list1)
    # subject_temp.coverage_summary()
    # subject_temp.strain_initial()
    # subject_temp.strain_identify(out_name1, out_name2, out_name3)
